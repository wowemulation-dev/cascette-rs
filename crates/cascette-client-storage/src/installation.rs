...
impl Installation {
    ...
    /// Flush in-memory index entries to disk.
    ///
    /// Merges pending update-section entries into the sorted section and
    /// writes the result to disk. Without this, entries accumulate in memory
    /// but the on-disk IDX files have empty sorted sections, forcing the
    /// client to rebuild indices on first startup.
