...
//! - `opt_in_feedback`: always true (default)
...
//! - `region`: region redirect (triggers CDN re-routing)
...
pub struct SetProductOverrideRequest {
    ...
    /// Platform override (e.g., "Windows", "OSX").
    pub platform: Option<String>,
    /// Architecture override (e.g., "x86_64", "arm64").
...
pub async fn post_product_override_state(
    ...
    // Store account_country, geo_ip_country, platform, and architecture on
    // the product record so they propagate to TagQuery and .product.db
    // during installation.
    if (body.account_country.is_some()
        || body.geo_ip_country.is_some()
        || body.platform.is_some()
        || body.architecture.is_some())
        && let Ok(mut p) = state.registry.get(&product_lower).await
    {
        if let Some(ref acct) = body.account_country {
            p.account_country = Some(acct.clone());
        }
        if let Some(ref geo) = body.geo_ip_country {
            p.geo_ip_country = Some(geo.clone());
        }
        if let Some(ref plat) = body.platform {
            p.platform = Some(plat.clone());
        }
        if let Some(ref arch) = body.architecture {
            p.architecture = Some(arch.clone());
        }
