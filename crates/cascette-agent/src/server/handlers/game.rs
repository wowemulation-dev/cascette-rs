...
///
/// Returns product metadata, current status, version, and the active
/// operation's progress (if any). The `progress` and `error` fields are
/// populated from the most recent active operation for this product.
pub async fn get_game(
    ...
    match state.registry.get(&product).await {
        Ok(p) => {
            // Look up active operation to get progress/error.
            let (progress, op_state, error) =
                match state.queue.find_active_for_product(&p.product_code).await {
                    Ok(Some(op)) => (op.progress, Some(op.state.to_string()), op.error),
                    _ => (None, None, None),
                };
...
///
/// The Blizzard Agent writes this file to persist launch parameters.
/// We write a JSON-encoded config matching the known fields.
