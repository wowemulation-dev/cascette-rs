...
/// GET /hardware
///
/// Returns hardware information in the Blizzard Agent wire format.
///
...
pub async fn get_hardware() -> Json<serde_json::Value> {
    ...
    let cpu_brand = sys
        ...
        .unwrap_or_default();
    // cpu_vendor: not directly exposed by sysinfo; use empty string since
    // this is only populated via CPUID on Windows.
    ...
    // cpu_arch: the protocol uses an enum (0=x86, 1=x64, etc.).
    ...
    // The protocol supports up to 3 GPUs (gpu_1, gpu_2, gpu_3).
...
mod tests {
    ...
    async fn get_hardware_returns_all_fields() {
        ...
        // Flat proto fields must all be present.
