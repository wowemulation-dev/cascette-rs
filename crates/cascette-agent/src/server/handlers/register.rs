...
///
/// Fields match the Blizzard Agent wire format. The `build_config`, `cdn_config`,
/// and `product_config` fields are cascette extensions not present in the
/// original protocol.
...
pub struct RegisterRequest {
    ...
    /// Protocol identifier. The Blizzard Agent requires exact `"NGDP"` match.
    ...
    /// Target platform override (cascette extension, e.g., "Windows", "OSX").
...
pub async fn post_register(
    ...
    // install_path is the base directory where Data/ (CASC storage) lives.
    // The subfolder is appended by the loose file handler, not here.
    // install_dir is stored as-is; the subfolder is only joined
    // when constructing the product (loose file) directory.
    let install_path = body.install_dir.clone();
    ...
    product.platform.clone_from(&body.platform);
    product.architecture.clone_from(&body.architecture);
