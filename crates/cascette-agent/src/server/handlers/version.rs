...
//!
//! The Blizzard Agent accepts an optional `uid` query parameter and returns a
//! single `version` string: `"TACT x.y.z (CASC a.b.c)"`. The uid is ignored;
//! the response is the same regardless of its value.
//!
...
//! These extensions are not present in the Blizzard Agent but do not break
//! callers that only read `version`.
