#!/usr/bin/env pwsh
# Install script for ngdp-client with ephemeral key verification
...
$BinaryName = "ngdp"
$Repo = "wowemulation-dev/cascette-rs"
...
function Get-LatestVersion {
    try {
        ...
        $version = $release.tag_name -replace 'ngdp-client-v', ''
        return $version
    } catch {
...
function Install-NgdpClient {
    ...
    Write-Info "Installing ngdp-client v$Version"
    ...
    try {
        # Download files
        $fileName = "ngdp-$Version-$platform.zip"
        $downloadUrl = "$BaseUrl/download/ngdp-client-v$Version/$fileName"
        ...
        $pubKeyDownloaded = Download-File -Url "$BaseUrl/download/ngdp-client-v$Version/minisign.pub" -OutFile $pubKeyFile
...
if ($Help) {
    Write-Host @"
Install script for ngdp-client from cascette-rs
...
OPTIONS:
    ...
    -InstallDir  Installation directory (default: $env:LOCALAPPDATA\Programs\ngdp-client)
...
Install-NgdpClient -Version $Version -InstallDir $InstallDir