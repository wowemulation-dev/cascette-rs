...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match client.download_file_range(cdn_host, path, test_hash, (0, Some(255))).await {
        Ok(response) => {
            ...
            println!("Downloaded {} bytes (requested first 256 bytes)", partial_data.len());
    ...
    match client.download_file(cdn_host, path, test_hash).await {
        Ok(response) => {
            ...
            match client.download_file_range(cdn_host, path, test_hash, (0, Some(127))).await {
                Ok(range_response) => {
                    let range_status = range_response.status();
                    let partial_data = range_response.bytes().await?;
                    println!("Partial download: {} bytes (requested first 128 bytes)", partial_data.len());
    ...
    for (i, &range) in ranges.iter().enumerate() {
        match client.download_file_range(cdn_host, path, test_hash, range).await {
            Ok(response) => {
                let data = response.bytes().await?;
                match range {
                    (start, Some(end)) => {
                        println!("Range {}: bytes {}-{} = {} bytes downloaded", 
                                i + 1, start, end, data.len());
                    },
                    (start, None) => {
                        println!("Range {}: bytes {}- (to end) = {} bytes downloaded", 
                                i + 1, start, data.len());
                    }
                }
            },
    ...
    match client.download_file_multirange(cdn_host, path, test_hash, &multi_ranges).await {
        Ok(response) => {
            ...
            if let Some(content_type) = response.headers().get("content-type") {
                ...
                if content_type.to_str().unwrap_or("").starts_with("multipart/byteranges") {
