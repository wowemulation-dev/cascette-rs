...
async fn test_basic_request_batching() {
    ...
    let results = client.download_batched("example.com", "data", &hashes).await;
...
async fn test_batch_statistics() {
    ...
    let _results = client.download_batched("example.com", "data", &hashes).await;
...
async fn test_batch_performance_characteristics() {
    ...
    let hashes: Vec<String> = (0..50)
        .map(|i| format!("hash{:04x}", i))
        .collect();
    
    let start = Instant::now();
    let results = client.download_batched("httpbin.org", "status/404", &hashes).await; // Will return 404s
...
async fn test_specialized_batch_methods() {
    ...
    // Test data batching
    let data_results = client.download_data_batched("example.com", "test", &hashes).await;
    assert_eq!(data_results.len(), hashes.len());
    
    // Test config batching  
    let config_results = client.download_config_batched("example.com", "test", &hashes).await;
    assert_eq!(config_results.len(), hashes.len());
    
    // Test patch batching
    let patch_results = client.download_patch_batched("example.com", "test", &hashes).await;
...
async fn test_batch_vs_parallel_comparison() {
    ...
    let hashes: Vec<String> = (0..20)
        .map(|i| format!("file{:04x}", i))
        .collect();
    
    // Test parallel downloads
    let start = Instant::now();
    let parallel_results = client.download_parallel("httpbin.org", "status/200", &hashes, Some(5)).await;
    let parallel_duration = start.elapsed();
    
    // Test batched downloads
    let start = Instant::now();
    let batch_results = client.download_batched("httpbin.org", "status/200", &hashes).await;
    let batch_duration = start.elapsed();
...
async fn test_batch_error_handling() {
    ...
    let results = client.download_batched("httpbin.org", "status/404", &hashes).await;
...
async fn benchmark_batch_vs_parallel() {
    ...
    let hashes: Vec<String> = (0..100)
        .map(|i| format!("benchmark{:06x}", i))
        .collect();
    
    println!("Benchmarking with {} files...", hashes.len());
    
    // Warm up
    let _ = client.download_batched("httpbin.org", "status/204", &hashes[0..5]).await;
    
    // Benchmark parallel
    let start = Instant::now();
    let _parallel_results = client.download_parallel("httpbin.org", "status/204", &hashes, Some(20)).await;
    let parallel_time = start.elapsed();
    
    // Benchmark batched
    let start = Instant::now();
    let _batch_results = client.download_batched("httpbin.org", "status/204", &hashes).await;
