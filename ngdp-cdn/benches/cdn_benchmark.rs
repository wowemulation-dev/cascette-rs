...
use ngdp_cdn::{Backoff, CdnClient, DummyCacheProvider, SingleHost, build_path};
...
fn benchmark_client_creation(c: &mut Criterion) {
    c.bench_function("client_builder", |b| {
        b.iter(|| {
            ...
            CdnClient::<_, DummyCacheProvider>::builder()
                .max_retries(5)
...
fn benchmark_backoff_calculation(c: &mut Criterion) {
    ...
    c.bench_function("calculate_backoff_0", |b| {
        b.iter(|| backoff.calculate_backoff(black_box(0)))
    });

    c.bench_function("calculate_backoff_5", |b| {
        b.iter(|| backoff.calculate_backoff(black_box(5)))
    });
}
