...
impl KeyService {
    ...
    pub fn new() -> Self {
        ...
        let mut service = Self { keys };

        // Try to load additional keys from standard directories
        match service.load_from_standard_dirs() {
            Ok(count) if count > 0 => {
                info!("Loaded {} additional keys from standard directories", count);
            }
            Ok(_) => {
                debug!("No additional keys found in standard directories");
            }
            Err(e) => {
                warn!("Failed to load keys from standard directories: {}", e);
            }
        }

        info!("Total keys available: {}", service.key_count());
