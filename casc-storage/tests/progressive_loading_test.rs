...
async fn test_size_hint_logic() {
    ...
    assert_eq!(
        high_confidence.suggested_initial_size(),
        Some(3 * 1024 * 1024)
    );
...
async fn test_progressive_loading_with_real_storage() {
    ...
    let chunk2 = progressive_file
        .read(100_000, 2048)
        .await
        .expect("Failed to read chunk 2");
    ...
    let chunk3 = progressive_file
        .read(end_offset as u64, 1024)
        .await
        .expect("Failed to read from end");
...
async fn test_progressive_cleanup_and_stats() {
    ...
    storage
        .write(&ekey1, &test_data1)
        .expect("Failed to write test data 1");
    storage
        .write(&ekey2, &test_data2)
        .expect("Failed to write test data 2");
    storage.flush().expect("Failed to flush storage");
    storage.load_archives().expect("Failed to load archives");
...
async fn test_progressive_vs_traditional_performance() {
    ...
    let start = std::time::Instant::now();
    let progressive_data = progressive_file
        .read(0, test_data.len())
        .await
        .expect("Progressive read failed");
    ...
    assert!(
        ratio < 3.0,
        "Progressive reading is too slow compared to traditional"
    );

    // Test partial read performance (where progressive should shine)
    let start = std::time::Instant::now();
    let partial_data = progressive_file
        .read(1_000_000, 4096)
        .await
    ...
    assert!(
        partial_time < traditional_time / 2,
        "Partial progressive read should be much faster"
    );
}
