...
fn test_large_archive_memory_mapping_limits() {
    ...
    assert!(reader.is_memory_mapped(), "Small files should be memory-mapped");
    ...
    #[cfg(target_pointer_width = "32")]
    {
        ...
        assert!(ArchiveReader::can_memory_map(1024 * 1024 * 1024), "1GB should be mappable on 32-bit");
        assert!(!ArchiveReader::can_memory_map(3 * 1024 * 1024 * 1024), "3GB should not be mappable on 32-bit");
    }
    ...
    #[cfg(target_pointer_width = "64")]
    {
        ...
        assert!(ArchiveReader::can_memory_map(8 * 1024 * 1024 * 1024), "8GB should be mappable on 64-bit");
        assert!(ArchiveReader::can_memory_map(64 * 1024 * 1024 * 1024), "64GB should be mappable on 64-bit");
        
        // But not infinitely large files
        let too_large = 200u64 * 1024 * 1024 * 1024; // 200GB
        assert!(!ArchiveReader::can_memory_map(too_large), "200GB should not be mappable to avoid VM exhaustion");
    }
}
        ...
        let handle = thread::spawn(move || {
            ...
            for (i, chunk) in data.chunks(4).enumerate() {
                ...
                assert_eq!(actual_value, expected_value, 
                    "Thread {} failed at position {}", thread_id, i);
            }
...
fn test_memory_mapping_vs_file_reading_performance() {
    ...
    assert!(reader.is_memory_mapped(), "Should use memory mapping for 1MB file");
...
fn test_bounds_checking() {
    ...
    assert!(result.is_err(), "Should fail when reading beyond file bounds");
...
fn test_large_archive_simulation() {
    ...
    for (i, sim_file) in simulated_files.iter().enumerate() {
        let read_data = reader.read_at(sim_file.offset, sim_file.size as usize).unwrap();
        assert_eq!(read_data, sim_file.content, "Simulated file {} content mismatch", i);
    }
    
    println!("Successfully tested {} simulated files in archive", simulated_files.len());
}