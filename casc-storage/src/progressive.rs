...
impl AccessPattern {
    ...
    fn record_access(&mut self, chunk_index: usize) {
        ...
        if let Some(last) = self.last_access {
            ...
            self.avg_interval = Some(match self.avg_interval {
                Some(avg) => Duration::from_nanos(((avg.as_nanos() + interval.as_nanos()) / 2).min(u64::MAX as u128) as u64),
                None => interval,
            });
        }
    ...
    fn predict_next_chunks(&self, current_chunk: usize, max_predictions: usize) -> Vec<usize> {
        ...
        // Analyze pattern
        let is_sequential = self.sequential_reads
            .iter()
            .collect::<Vec<_>>()
            .windows(2)
            .all(|w| w[1] == &(w[0] + 1));
...
impl ProgressiveFile {
        ...
        for chunk_index in start_chunk..=end_chunk {
            ...
            let chunk_data = if let Some(data) = chunk_data {
                ...
                chunks.get(&chunk_index)
                    .ok_or_else(|| CascError::InvalidArchiveFormat("Chunk failed to load".to_string()))?
                    .data.clone()
            };
    ...
    async fn load_chunk(&self, chunk_index: usize, priority: ChunkPriority) -> Result<()> {
        ...
        self.chunk_request_tx.send(request)
    ...
    pub async fn is_fully_loaded(&self) -> bool {
        if let SizeHint::Exact(size) = self.size_hint {
            ...
            chunks.len() == expected_chunks && 
                chunks.values().any(|chunk| chunk.is_final)
        } else {
            false
        }
    }
    ...
    async fn load_single_chunk(
        ...
        match loader.load_chunk(ekey, chunk_offset, config.chunk_size).await {
            Ok(chunk_data) => {
                ...
                trace!(
                    ...
                    chunk_index, chunk.size, ekey, start_time.elapsed()
                );
...
impl ProgressiveFileManager {
    ...
    pub async fn get_or_create_progressive_file(
        ...
        info!("Created progressive file for {} with hint {:?}", ekey, size_hint);
        progressive_file
    }
...
mod tests {
    ...
    async fn test_progressive_file_creation() {
        ...
        let file = manager.get_or_create_progressive_file(ekey, size_hint).await;
