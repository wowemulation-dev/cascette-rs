...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let samples = vec![
        (
            "small_file",
            create_sample_data(256 * 1024, "Small file content"),
        ), // 256KB - won't use progressive
        (
            "medium_file",
            create_sample_data(1 * 1024 * 1024, "Medium file content"),
        ), // 1MB - will use progressive
        (
            "large_file",
            create_sample_data(5 * 1024 * 1024, "Large file content"),
        ), // 5MB - will use progressive
    ];
    ...
    for (name, ekey, size) in &ekeys {
        ...
        let should_use_progressive =
            size_hint.should_use_progressive(&storage.progressive_manager.as_ref().unwrap().config);
    ...
    if let Some((_, large_ekey, large_size)) =
        ekeys.iter().find(|(name, _, _)| name == &"large_file")
    {
...
async fn await_progressive_loading_demo(
    ...
    println!(
        "  📊 Progressive file stats: chunks_loaded={}, bytes_loaded={}, cache_hits={}, cache_misses={}",
...
async fn await_concurrent_progressive_demo(
    ...
    let progressive_file = storage
        .read_progressive(ekey, SizeHint::Exact(file_size as u64))
        .await?;
    ...
    let (result1, result2, result3) = tokio::join!(
        async move {
            let data = file1
                .read(0, 16 * 1024)
                .await
                .expect("Concurrent read 1 failed");
            (data.len(), "beginning")
        },
        async move {
            let offset = file_size / 3;
            let data = file2
                .read(offset as u64, 16 * 1024)
                .await
                .expect("Concurrent read 2 failed");
            (data.len(), "middle")
        },
        async move {
            ...
            let data = file3
                .read(offset as u64, 16 * 1024)
                .await
                .expect("Concurrent read 3 failed");
            (data.len(), "end")
        }
    );
    ...
    println!(
        "  📊 Final concurrent stats: chunks_loaded={}, total_time={:?}, avg_chunk_time={:?}",
