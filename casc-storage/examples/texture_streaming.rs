...
impl TextureManager {
    ...
    async fn load_texture_header(&mut self, ekey: &EKey) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
...
async fn simulate_texture_loading() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(ekey) = files.first() {
        ...
        for level in 0..3 {
            ...
            match texture_manager.load_texture_mipmap(ekey, offset, size).await {
                Ok(data) => {
    ...
    for (ekey, stat) in stats {
        let cache_ratio = stat.cache_hits as f64 / (stat.cache_hits + stat.cache_misses).max(1) as f64;
        info!(
