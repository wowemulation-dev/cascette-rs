...
impl EncodingTableHeader {
    /// Parse an encoding table header.
    ...
    /// Asynchronously parse an encoding table header.
    pub async fn aparse<R: AsyncBufReadExt + Unpin>(f: &mut R) -> Result<Self> {
        let mut magic = [0; 2];
        ...
        Ok(Self {
            version,
            hash_size_ckey: f.read_u8().await?,
            hash_size_ekey: f.read_u8().await?,
            ce_key_page_table_page_size: u32::from(f.read_u16().await?) * 1024,
            e_key_spec_page_table_page_size: u32::from(f.read_u16().await?) * 1024,
            ce_key_page_table_page_count: f.read_u32().await?,
            e_key_spec_page_table_page_count: f.read_u32().await?,
            flags: f.read_u8().await?,
            e_spec_block_size: f.read_u32().await?,
        })
    }
}
...
impl<const CKEY_SIZE: usize, const EKEY_SIZE: usize> CEKeyEntry<CKEY_SIZE, EKEY_SIZE> {
    ...
    /// Asynchronously a single `CEKeyPageTable` entry.
    pub async fn aparse<R: AsyncReadInt + AsyncBufReadExt + Unpin>(
        f: &mut R,
        remain: &mut u32,
    ) -> Result<Option<Self>> {
        if *remain < Self::BASE_ENTRY_SIZE as u32 + EKEY_SIZE as u32 {
        ...
        let file_size = f.read_u40().await?;
        let mut ckey = [0; CKEY_SIZE];
        f.read_exact(&mut ckey).await?;

        let mut ekeys = Vec::with_capacity(usize::from(count));
        for _ in 0..count {
            let mut ekey = [0; EKEY_SIZE];
            f.read_exact(&mut ekey).await?;
            ekeys.push(ekey);
        }
...
impl EncodingTable {
    ...
    /// Asynchronously parse an encoding table.
    pub async fn aparse<R: AsyncBufReadExt + AsyncSeekExt + Unpin>(f: &mut R) -> Result<Self> {
        let header = EncodingTableHeader::aparse(f).await?;
        ...
        // Skip the espec BLTE strings
        f.seek(SeekFrom::Current(header.e_spec_block_size.into()))
            .await?;
        ...
        for _ in 0..header.ce_key_page_table_page_count {
            // Skip first_ce_key (we don't use this)
            f.seek(SeekFrom::Current(MD5_LENGTH as i64)).await?;

            let mut page_md5 = [0; MD5_LENGTH];
            f.read_exact(&mut page_md5).await?;
            ce_key_page_table_md5.push(page_md5);
        }
        ...
        for (i, page_md5) in ce_key_page_table_md5.iter().enumerate() {
            ...
            let mut page = Cursor::new(&page);
            let mut remain = header.ce_key_page_table_page_size;
            while let Some(entry) = CEKeyEntry::aparse(&mut page, &mut remain).await? {
                if let Some(old) = md5_map.insert(entry.ckey, (entry.file_size, entry.ekeys)) {
                    warn!(
                        "Encoding key conflict for {}: {old:?}",
                        hex::encode(entry.ckey),
