...
vfs-3 = 
    ...
    assert_eq!(
        config.encoding_hash(),
        Some("9e3f7e6dc5e526ad88d14332fecb6a12")
    );
    assert_eq!(
        config.install_hash(),
        Some("79e1afb713f96ca3e9f049aca3f1b433")
    );
    assert_eq!(
        config.download_hash(),
        Some("fb5ba2d2eef871e31d28c73e5d883754")
    );
    ...
    assert_eq!(
        config.config.get_value("build-uid"),
        Some("wow_classic_era")
    );
...
patch-file-index-size = 1de5736c18db6e6bb3496fe635876dc8 2376
    ...
    assert_eq!(
        ...
        Some("fb3c60af492e4bc4863e323d087e7166")
    );

    // Test file index
    assert_eq!(
        config.file_index(),
        Some("eb439ef75c96c973c0c711117b76e61f 17024")
    );
...
empty-value = 
    ...
    assert_eq!(
        config.get_value("quoted-value"),
        Some("\"value with spaces\"")
    );
    assert_eq!(
        config.get_value("path-value"),
        Some("C:\\Path\\To\\File.dat")
    );
    assert_eq!(
        config.get_value("url-value"),
...
key5 = 中文字符
    ...
    assert_eq!(
        config.get_value("key2"),
        Some("value with multiple   spaces")
    );
    assert_eq!(config.get_value("key3"), Some("value	with	tabs"));
    assert_eq!(
        config.get_value("key4"),
        Some("!@#$%^&*()_+-=[]{}|;':\",./<>?")
    );
...
invalid-hash = ghijkl 456
    ...
    assert_eq!(
        config.get_value("multi-equals"),
        Some("value = with = many = equals")
    );
