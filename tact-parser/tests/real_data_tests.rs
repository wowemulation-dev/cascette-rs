...
async fn download_cdn_file(cdn_host: &str, cdn_path: &str, hash: &str) -> Vec<u8> {
    ...
    response.bytes().await.expect("Failed to read bytes").to_vec()
}
...
async fn get_build_info_for_product(product: &str) -> (String, String, String, String, String) {
    ...
    let versions_response = ribbit_client.request_raw(&versions_endpoint).await
    ...
    let bpsv = ngdp_bpsv::BpsvDocument::parse(versions_str)
        .expect("Failed to parse versions");
    
    // Find US entry
    let us_row = bpsv.rows().iter()
        .find(|row| {
            row.raw_values().get(0).map(|v| *v == "us").unwrap_or(false)
        })
    ...
    let cdns_response = ribbit_client.request_raw(&cdns_endpoint).await
        .expect("Failed to get CDNs");
    
    let cdns_str = std::str::from_utf8(&cdns_response).expect("Invalid UTF-8");
    let cdns_bpsv = ngdp_bpsv::BpsvDocument::parse(cdns_str)
        .expect("Failed to parse CDNs");
    
    let cdn_row = cdns_bpsv.rows().iter()
        .find(|row| {
            row.raw_values().get(0).map(|v| *v == "us").unwrap_or(false)
        })
    ...
    println!("{} Build config: {}", product, build_config_hash);
    println!("{} CDN config: {}", product, cdn_config_hash);
    println!("{} CDN host: {}", product, cdn_host);
    println!("{} CDN path: {}", product, cdn_path);
    
    (cdn_host, cdn_path, build_config_hash, cdn_config_hash, product.to_string())
}
...
async fn test_real_build_config() {
    ...
    println!("\nAnalysis:");
    println!("root value: {:?}", build_config.config.get_value("root"));
    println!("encoding value: {:?}", build_config.config.get_value("encoding"));
    println!("install value: {:?}", build_config.config.get_value("install"));
    println!("download value: {:?}", build_config.config.get_value("download"));
    println!("size value: {:?}", build_config.config.get_value("size"));
    
    println!("\nHash extraction:");
    println!("Has root hash: {}", build_config.root_hash().is_some());
    println!("Has encoding hash: {}", build_config.encoding_hash().is_some());
    println!("Has install hash: {}", build_config.install_hash().is_some());
    println!("Has download hash: {}", build_config.download_hash().is_some());
    println!("Has size hash: {}", build_config.size_hash().is_some());
    ...
    println!("Build name: {:?}", build_config.build_name());
...
async fn test_real_cdn_config() {
    ...
    println!("Number of archives: {}", archives.len());
...
async fn test_real_download_manifest() {
    ...
    let build_config = BuildConfig::parse(&build_config_text).expect("Failed to parse build config");
    ...
    if let Some(download_hash) = build_config.download_hash() {
        ...
        assert!(!download_data.is_empty(), "Download manifest should not be empty");
        println!("Download manifest size (compressed): {} bytes", download_data.len());
...
async fn test_real_size_file() {
    ...
    if let Some(size_hash) = build_config.size_hash() {
        ...
        println!("Size file size (compressed): {} bytes", size_data.len());
...
async fn test_real_encoding_file() {
    ...
    let encoding_hash = build_config.encoding_hash().expect("Should have encoding hash");
    ...
    assert!(!encoding_data.is_empty(), "Encoding file should not be empty");
    println!("Encoding file size (compressed): {} bytes", encoding_data.len());
...
async fn test_real_install_manifest() {
    ...
    if let Some(install_hash) = build_config.install_hash() {
        ...
        assert!(!install_data.is_empty(), "Install manifest should not be empty");
        println!("Install manifest size (compressed): {} bytes", install_data.len());
...
async fn test_compare_config_formats() {
    ...
    let (cdn_host_classic, cdn_path_classic, build_config_hash_classic, _, _) = get_build_info_for_product("wow_classic_era").await;
    
    // Download and parse classic build config
    let url_classic = format!("http://{}/{}/config/{}/{}/{}", 
        cdn_host_classic, 
        cdn_path_classic,
    ...
    let response_classic = reqwest::get(&url_classic).await.expect("Failed to download");
    ...
    let (cdn_host_retail, cdn_path_retail, build_config_hash_retail, _, _) = get_build_info_for_product("wow").await;
    
    // Download and parse retail build config
    let url_retail = format!("http://{}/{}/config/{}/{}/{}", 
        cdn_host_retail, 
        cdn_path_retail,
    ...
    // Parse both configs
    let build_config_classic = BuildConfig::parse(&data_classic).expect("Failed to parse classic config");
    let build_config_retail = BuildConfig::parse(&data_retail).expect("Failed to parse retail config");
