...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    v2_client.inner_mut().set_protocol_version(ribbit_client::ProtocolVersion::V2);
    ...
    if let Some(data) = &v2_response.data {
        ...
        println!("   Data preview: {}...", &data.chars().take(50).collect::<String>());
    }
    ...
    println!("   V2 Response:");  
    println!("     - Has raw data: {}", v2_response.raw.is_empty() == false);
    println!("     - Has parsed data: {}", v2_response.data.is_some());
    println!("     - Has MIME parts: {}", v2_response.mime_parts.is_some());
    ...
    if let Some(first) = typed.entries.first() {
        println!("   First entry: {} - Build {}", first.region, first.build_id);
    }
