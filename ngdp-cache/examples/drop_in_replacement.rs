...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(us_version) = versions.entries.iter().find(|e| e.region == "us") {
        println!(
            "   US Build: {} ({})",
            us_version.build_id, us_version.versions_name
        );
    }
    ...
    let bgdl = client
        .request_typed::<ribbit_client::ProductBgdlResponse>(&Endpoint::ProductBgdl(
            "wow".to_string(),
        ))
        .await?;
    ...
    let raw = client
        .request_raw(&Endpoint::ProductVersions("d4".to_string()))
        .await?;
