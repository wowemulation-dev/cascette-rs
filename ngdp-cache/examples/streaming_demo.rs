...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let start = Instant::now();
    let mut output = Vec::new();
    cache.read_streaming_buffered("copied_demo", &mut output, 64 * 1024).await?; // 64KB buffer
...
async fn demonstrate_progress_tracking(cache: &GenericCache) -> Result<(), Box<dyn std::error::Error>> {
    ...
    cache.read_chunked("progress_demo", |chunk| {
